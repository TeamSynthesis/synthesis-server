...
public static class GptSystemMessages
{
    ...
    private const string FeaturesResponseFormat = @"[{""Name"":"""",""Description"":"""",""Type"":null,""Tasks"":[{""Activity"":"""",""Priority"":null,""memberId"":""00000000-0000-0000-0000-000000000000""}]}]";
    ...
    public static string GetOverviewPrompt()
    {
        return $@"
                    ...
                    Today, I need your assistance in generating a comprehensive project overview document in JSON format for an innovative A software project  idea i will pass as a prompt. For optimal accuracy and relevance, 
                    Do not include any explanations, only provide a  RFC8259 compliant minified JSON response  following this format without deviation.
    ...
    public static string GetProjectCompetitiveAnalysisPrompt()
    {
        return $@"
                    ...
                    Today, I need your assistance in generating a comprehensive project competitive analysis document in JSON format for an innovative A software project  idea i will pass as a prompt. For optimal accuracy and relevance, 
                    Do not include any explanations, only provide a  RFC8259 compliant minified JSON response  following this format without deviation.
    ...
    public static string GetFeaturesPrompt(List<GptTeamMemberDto> teamMembers)
    {
        var addTaskAssingmentPromptInjection = string.Empty;

        if (teamMembers == null || teamMembers.Count == 0)
        {
            addTaskAssingmentPromptInjection = "use the default memberId: 00000000-0000-0000-0000-000000000000 for each task ";
        }
        else
        {
            addTaskAssingmentPromptInjection = $"from these team members: {System.Text.Json.JsonSerializer.Serialize(teamMembers)} you are to set the  task memberId to  member Id of the team mem ber who can fit thasks based on their skills, if a task is not assigned to a team member us, ife default memberId: 00000000-0000-0000-000-000000000000 try to find the best task fit for a team member tsed on their skills";
        }
        return $@"
                   You are a software project management assistant trained on a massive dataset of project documentation, industry research, and expert interviews. Your core function is to help startup devs, and project managers like me efficiently comeup detailed project features and tasks based on your insight.
                    ...
                    Today, I need your assistance in generating a comprehensive project features document in JSON format for an innovative software project idea i will pass as a prompt. For optimal accuracy and relevance, 
                    Do not include any explanations, only provide a  RFC8259 compliant minified JSON response  following this format without deviation.
                    {FeaturesResponseFormat}
                    
                    
                  
                    {addTaskAssingmentPromptInjection}
    ...
    public static string GetBrandingPrompt()
    {
        return $@"
                    ...
                    For optimal accuracy and relevance, Do not include any explanations, only provide a  RFC8259 compliant minified JSON response  following this format without deviation.
                    ...
                    wireframe: you are to generate an image generation prompt for dalle to use to create a realistic
                     ...
                     ireframe featuring the primary, secondary and accent colors suggested in the palette, for a screen in the app., 
